...
type AxesT         = str | list[list[int | float]] | np.ndarray | None
type ElemT         = (
...
type PosT          = (
    list[int | list[int | float | np.float64]]
    | np.ndarray
    | None
    )
type TilingT       = (
    list[int | list[int] | np.float64]
    | np.ndarray
    | tuple[int, int, int]
    | None
    )
type BcondT        = list[int | str] | tuple[str, str, str] | None
type ElemT2        = (
    list[int | str | list[int | float]]
    | np.ndarray
    | tuple[str, str]
    )
type PosT2         = list[int | list[int | float | np.float64]] | np.ndarray
type RotateArg     = str | list[int | float | list[float]] | np.ndarray
type RotateArg2    = float | str | list[int] | np.ndarray | None
type IdentifiersT  = int | str | Structure | list[int | str] | np.ndarray
type UniquePoinRet = (
    tuple[np.ndarray | np.uint64, np.ndarray | np.uint64 | None, obj | np.uint64]
    )
type RmgLatticeRet = (
    str
    | tuple[str | None, np.ndarray | None, Structure, Structure, str]
    | tuple[str | None, str | np.ndarray | None]
    | None
    )
type GetKpathRet   = (
    dict[str, dict[str, list[float]] | list[str | tuple[str, str]] | np.ndarray]
    )
type TilingT2      = (
    list[int | list[int] | np.float64]
    | tuple[int, int, int]
...
type KshiftT       = tuple[int | float, int | float, int | float]
...
def read_cif_celldata(
    ...
    *,
    ...
    cb = cf.get(block)
    if cb is None:
        msg = f'block {block} was not found in cif file {filepath}'
...
def read_cif(
    ...
    *,
    ...
    units = 'A'
    axes  = scale*np.array(cell.latticevectors,dtype=float)
    elem  = []
    pos   = []
    for wyckoff_atoms in cell.atomdata:
        ...
        #end for
    #end for
    pos = dot(np.array(pos,dtype=float),axes)
...
def kmesh(
    kaxes : np.ndarray,
    dim   : list[int] | tuple[int, int, int],
    shift : ShiftT = None,
    ) -> np.ndarray:
    '''
...
def reduce_tilematrix(
    ...
    if np.abs(tiling-t).sum()>1e-6:
        msg = (
            'requested tiling is non-integer\n'
    ...
    if matrix_tiling:
        if np.abs(det(t))==0:
            msg = (
                'requested tiling matrix is singular\n'
                f'tiling requested: {t}'
                )
            raise ValueError(msg)
        #end if
        ...
        tilematrix = T.copy()
        del t
        tbar = np.identity(dim) #basis for shearing
        dr = list(range(dim))
        #dr = [1,0,2]
        ...
        for dp in [(0,1,2),(2,0,1),(1,2,0),(2,1,0),(0,2,1),(1,0,2)]:
            success = True
            Tnew = np.array(T,dtype=float) #sheared/orthogonal tiling matrix
            for d in dr:
                tb = tbar[dp[d]]
                t  = T[d]
                d2,d3 = other[d]
            ...
            if success:
                ...
                if nondiagonal:
                    msg = 'could not find a diagonal tiling matrix for generating tiled coordinates'
        ...
        for tv in tvecs_old:
            ...
            #end if
        #end for
        tilevector = np.array(tvecs)
    else:
        tilevector = t
        tilematrix = np.diag(t)
    #end if
...
def rotate_plane(
    ...
    if plane=='xy':
        ...
        msg = (
            'plane must be xy/yx/yz/zy/zx/xz\n'
            f'you provided: {plane}'
            )
        raise ValueError(msg)
    #end if
...
def trivial_filter(T: np.ndarray) -> bool:
    return True
#end def trival_filter

class MaskFilter(DevBase):
    #mth
    def set(self, mask, dim: int = 3) -> None:
        omask = np.array(mask)
        ...
        if mask.size==dim:
            ...
            msg = (
                f"shape of mask array must be {dim},{dim}\n"
                f"shape received: {mask.shape[0]},{mask.shape[1]}\n"
                f"mask array received: {omask}"
                )
            raise ValueError(msg)
        #end if
...
def optimal_tilematrix(
    ...
    if Taxopt is None:
        msg = (
            f"optimal tilematrix for volfac={volfac} not found with tolerance {tol}\n"
            f"difference range (dn): {dn}\n"
            f"tiling matrices searched: {ntilings}\n"
            f"cells with target volume: {nequiv_volume}\n"
            f"cells that passed the filter: {nfilter}\n"
            f"cells with equivalent inscribing radius: {nequiv_inscribe}\n"
            f"cells with equivalent wigner radius: {nequiv_wigner}\n"
            f"cells with equivalent cubicity: {nequiv_cubicity}\n"
            f"matrices with equivalent shapeliness: {nequiv_shape}\n"
            f"please try again with dn={dn+1}"
            )
        raise RuntimeError(msg)
    #end if
    if det(Taxopt)<0:
...
class Structure(Sobj):
    ...
    def check_consistent(
        ...
        *,
        ...
        if self.has_axes():
            ...
            abs_diff = np.abs(self.kaxes-kaxes).sum()
            if abs_diff>tol:
                msg += (
                    "Direct and reciprocal space axes are not consistent.\n"
                    "axes present:\n"
                    f"{self.axes}\n"
                    "kaxes present:\n"
                    f"{self.kaxes}"
        ...
        if self.pos.shape!=pshape:
            msg += (
                "pos is not the right shape\n"
                f"pos shape: {self.pos.shape}\n"
                f"Correct shape: {pshape}\n"
                )
        #end if
        if self.mag is not None and len(self.mag)!=N:
            msg += (
                "mag does not have the right length\n"
                f"mag length: {self.mag}\n"
                f"Correct length: {N}\n"
                )
        #end if
        ...
        if self.frozen is not None and self.frozen.shape!=pshape:
            msg += (
                ...
                f"frozen shape: {self.frozen.shape}\n"
                f"Correct shape: {pshape}\n"
                )
        #end if
    ...
    def set_axes(self, axes: np.ndarray) -> None:
    ...
    def set_elem(self, elem: ElemT2) -> None:
        self.elem = np.array(elem,dtype=object)
    #end def set_elem


    def set_pos(self, pos: PosT2) -> None:
        self.pos = np.array(pos,dtype=float)
        if len(self.pos)!=len(self.elem):
            msg = (
                "Atomic positions must have same length as elem.\n"
                f"elem length: {len(self.elem)}\n"
                f"Atomic positions length: {len(self.pos)}\n"
                )
            raise ValueError(msg)
        #end if
    #end def set_pos
    ...
    def set_mag(self, mag: list[int | float] | None = None) -> None:
        if mag is None:
            self.mag = None
        else:
            ...
            if len(self.mag)!=len(self.elem):
                msg = (
                    ...
                    f"elem length: {len(self.elem)}\n"
                    f"Magnetic moments length: {len(self.mag)}\n"
                    )
                raise ValueError(msg)
            #end if
        #end if
        ...
        if vel is None:
            ...
            if self.vel.shape!=self.pos.shape:
                msg = (
                    ...
                    f"Positions shape: {self.pos.shape}\n"
                    f"Velocity shape: {self.vel.shape}"
                    )
                raise ValueError(msg)
            #end if
        #end if
    ...
    def set_frozen(self, frozen: list[list[bool]] | None = None) -> None:
        if frozen is None:
            self.frozen = None
        else:
            ...
            if self.frozen.shape!=self.pos.shape:
                msg = (
                    "Frozen directions must have the same shape as positions.\n"
                    f"Positions shape: {self.pos.shape}\n"
                    f"Frozen directions shape: {self.frozen.shape}"
                    )
                raise ValueError(msg)
            #end if
        #end if
    ...
    def operate(self, operations: obj) -> None:
        if isinstance(operations,obj):
            operations = operations.values()
        for op in operations:
            if op not in self.operations:
                msg = (
                    f"{op} is not a known operation\n"
                    "valid options are:\n"
    ...
    def set_folded(self, folded: Structure) -> None:
    ...
    def remove_folded_structure(self) -> None:
    ...
    def has_folded_structure(self) -> bool:
    ...
    # test needed
    def adjust_axes(self, axes: np.ndarray) -> None:
    ...
    # test needed
    #mth
    def reshape_axes(self, reshaping) -> None:
        ...
        if np.abs(np.abs(det(R))-1)<1e-6:
            ...
            if np.abs(np.abs(det(R))-1)<1e-6:
                ...
                msg = (
                    "reshaping matrix must not change the volume\n"
                    "  reshaping matrix:\n"
                    f"  {R}\n"
                    f"  volume change ratio: {np.abs(det(R))}"
                    )
                raise ValueError(msg)
            #end if
        #end if
    ...
    def write_axes(self) -> str:
        ...
        c = ''
        for a in self.axes:
            c+=f'{a[0]:12.8f} {a[1]:12.8f} {a[2]:12.8f}\n'
        #end for
        return c
    #end def write_axes
    ...
    def project_plane(
        ...
        #end for
        return np.array(plane_coords,dtype=float)
    ...
    def bounding_box(
        ...
        *,
        ...
        if box=='tight':
            ...
            prmax = prange.max()
            axes = np.diag((prmax,prmax,prmax))
        elif isinstance(box,np.ndarray) or isinstance(box,list):
            box = np.array(box)
            if box.shape!=(3,3):
                msg = (
    ...
    def center_molecule(self, rmin_edge: float = 1e-8) -> None:
        ...
        if hasattr(rmin_edge, '__len__'):
            ...
            if rmin_edge.shape != (self.dim,):
                msg = 'rmin_edge must have length '+str(self.dim)
                raise ValueError(msg)
        else:
        ...
        if lower_out or upper_out:
            msg = 'molecule does not fit in the cell'
            raise ValueError(msg)
        ...
        if lower_out or upper_out:
            msg = 'molecule is too close to the cell edges'
            raise ValueError(msg)
    ...
    # test needed
    #mth
    def permute(self, permutation) -> None:
        dim = self.dim
        P = np.empty((dim,dim),dtype=int)
        if len(permutation)!=dim:
            msg = (
                f" permutation vector must have {dim} elements\n"
                f" you provided {permutation}"
                )
            raise ValueError(msg)
        #end if
        for i in range(dim):
            p = permutation[i]
            pv = np.zeros((dim,),dtype=int)
            if p=='x' or p=='0':
                pv[0] = 1
            elif p=='y' or p=='1':
                pv[1] = 1
    ...
    def upcast(self, DerivedStructure):
        if not issubclass(DerivedStructure,Structure):
            msg = DerivedStructure.__name__+' is not derived from Structure'
            raise TypeError(msg)
        #end if
        ds = DerivedStructure()
    ...
    # test needed
    #mth
    def incorporate(self, other) -> None:
    ...
    # test needed
    def clone_from(self, other: Structure) -> None:
        if not isinstance(other,Structure):
            msg = (
                "cloning failed\n"
                "can only clone from other Structure objects\n"
                f"received object of type: {other.__class__.__name__}"
                )
            raise TypeError(msg)
        #end if
        o = deepcopy(other)
    ...
    # test needed
    def add_atoms(self, elem: list[str], pos: list[np.ndarray]) -> None:
    ...
    # test needed
    #mth
    def distances(self, pos1: Structure | None = None, pos2 = None):
        ...
        if len(pos1)!=len(pos2):
            msg = 'positions arrays are not the same length'
            raise ValueError(msg)
        #end if
    ...
    def count_kshells(
        self,
        kcut    : float,
        tilevec : tuple[int, int, int] = (12, 12, 12),
        nkdig   : int                  = 10,
        ) -> int:
      # check tilevec input
      for nt in tilevec:
    ...
    def volume(self) -> np.float64 | None:
        if not self.has_axes():
            return None
        else:
            return np.abs(det(self.axes))
        #end if
    #end def volume


    def rwigner(self, nc: int = 5) -> np.float64:
        if self.dim!=3:
            msg = 'rwigner is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        rmin = 1e90
        n=np.empty((1,3))
    ...
    def rinscribe(self) -> np.float64:
        if self.dim!=3:
            msg = 'rinscribe is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        radius = 1e99
        dim=3
        ...
        for d in range(dim):
            ...
            radius = min(radius,.5*np.abs(det(axes))/norm(rc))
        #end for
        return radius
    #end def rinscribe
    ...
    # test needed
    #mth
    def rwigner_cube(self, *args, **kwargs) -> float:
        cube = Structure()
        a = self.volume()**(1./3)
    ...
    # test needed
    #mth
    def rinscribe_cube(self, *args, **kwargs) -> float:
        cube = Structure()
        a = self.volume()**(1./3)
        cube.set_axes([[a,0,0],[0,a,0],[0,0,a]])
    ...
    def cube_deviation(self) -> float:
        a = self.axes
        dc = self.volume()**(1./3)*sqrt(2.)
        d1 = np.abs(norm(a[0]+a[1])-dc)
        d2 = np.abs(norm(a[1]+a[2])-dc)
        d3 = np.abs(norm(a[2]+a[0])-dc)
        d4 = np.abs(norm(a[0]-a[1])-dc)
        d5 = np.abs(norm(a[1]-a[2])-dc)
        d6 = np.abs(norm(a[2]-a[0])-dc)
    ...
    def unsheared_axes(
        ...
        *,
        ...
        if self.dim!=3:
            msg = 'unsheared_axes is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        if axes is None:
            axes = self.axes
        #end if
        dim=3
        axbar = np.identity(dim)
        axnew = np.array(axes,dtype=float)
        dists = np.empty((dim,))
        for d in range(dim):
            d2 = (d+1)%dim
            d3 = (d+2)%dim
    ...
    def stretch(
        ...
        if self.dim!=3:
            msg = 'stretch is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        d = np.diag((s1,s2,s3))
        self.skew(d)
        ...
        *,
        ...
        if rp is not None:
            ...
            if isinstance(r,str):
                ...
                r = np.array(r,dtype=float)
                if len(r.shape)>1:
                    msg = 'r must be given as a 1-d vector or string, if rp is not None'
                    raise ValueError(msg)
                #end if
            #end if
            if isinstance(rp,(int,float)):
                ...
                c = np.dot(r,rp)/norm(r)/norm(rp)
                if np.abs(c-1)<1e-6:
                    s = 0.0
                    r = np.array([1,0,0])
                else:
                    s = np.dot(cross(r,rp),cross(r,rp))/norm(r)/norm(rp)/norm(cross(r,rp))
                    r = cross(r,rp)/norm(cross(r,rp))
                #end if
            #end if
            # make R from r,theta
        ...
        #end if
        if check:
            if not np.allclose(dot(R,R.T),np.identity(len(R))):
                msg = 'the function, rotate, must be given an orthogonal matrix'
                raise ValueError(msg)
            #end if
        #end if
    ...
    # test needed
    def matrix_transform(self, A: np.ndarray) -> None:
        """
    ...
    # test needed
    def skew(self, skew: np.ndarray) -> None:
        """
    ...
    def cleave(
        ...
        *,
        ...
        if isinstance(axis,int):
            if sep is None:
                msg = 'separation induced by cleave must be provided'
                raise ValueError(msg)
            #end if
            ...
            v = axis
        #end if
        c = np.array(c)  # point on cleave plane
        v = np.array(v)  # normal vector to cleave plane, norm is cleave separation
        if sep is not None:
            v = np.abs(sep)*v/norm(v)
        #end if
        if norm(v)<tol:
            return
        #end if
        vn = np.array(v/norm(v))
        ...
        if self.has_axes():
            ...
            for i in range(dim):
                ...
                comp = np.abs(dot(a2,vn))+np.abs(dot(a3,vn))
                if comp < 1e-6:
                    components+=1
                    iaxis = i
                #end if
    ...
    # test needed
    #mth
    def locate_simple(self, pos) -> np.ndarray | tuple:
        pos = np.array(pos)
    ...
    def locate_by_elements(
        ...
        *,
        ...
        if not isinstance(elements, list | tuple | np.ndarray):
            ...
            msg = "Can not use an empty list/tuple/array for locating atoms!"
            raise ValueError(msg)
    ...
    def freeze(
        ...
        *,
        ...
        #end if
        if len(indices)==0:
            msg = 'failed to select any atoms to freeze'
            raise RuntimeError(msg)
        #end if
        if isinstance(directions,str):
            ...
            directions = np.array(directions,dtype=bool)
        #end if
        if self.frozen is None:
            self.frozen = np.zeros(self.pos.shape,dtype=bool)
        #end if
        frozen = self.frozen
        i=0
        if not negate:
    ...
    def is_frozen(self) -> bool | np.ndarray:
        if self.frozen is None:
    ...
    def is_magnetic(self, tol: float = 1e-8) -> bool:
        magnetic = False
        if self.mag is not None:
            for m in self.mag:
                if m is not None and np.abs(m)>tol:
                    magnetic = True
                    break
                #end if
            #end for
    ...
    # test needed
    def carve(self, identifiers: Structure) -> Structure:
        ...
        #end if
        sub.host_indices = np.array(indices)
        return sub
    #end def carve
    ...
    # test needed
    #mth
    def remove(self, identifiers) -> tuple:
        indices = self.locate(identifiers)
    ...
    # test needed
    #mth
    def replace_nearest(self, elem: Structure, pos = None) -> None:
        if isinstance(elem,Structure):
            cell = elem
            elem = cell.elem
            pos  = cell.pos
        ...
        if len(insert)>0:
            ...
            self.pos = np.array( list(self.pos[0:ii])+list(pos[insert])+list(self.pos[ii:]) )
        #end if
        self.remove_folded_structure()
    ...
    def point_defect(
        ...
        if identifiers is None:
            ...
            if index>=len(self.pos):
                msg = (
                    f"attempted to add a point defect at index {index}, which does not exist\n"
                    f"  for reference there are {len(self.pos)} atoms in the structure"
                    )
                raise ValueError(msg)
            #end if
        else:
            ...
            if len(indices)>1:
                msg = (
                    f"{len(indices)} atoms were located by identifiers provided\n"
                    "  a point defect replaces only a single atom\n"
                    f"  atom indices located: {indices}"
                    )
                raise RuntimeError(msg)
            #end if
            index = indices[0]
    ...
    def reorder(self, order: list[int] | np.ndarray) -> None:
        ...
        if not np.issubdtype(order.dtype,np.integer):
            msg = 'atom reordering indices must be integers'
            raise TypeError(msg)
        #end if
    ...
    def layers(
        ...
        *,
        ...
        naxis = vaxis/daxis
        dbin  = dtol/nbox
        nbins = int(np.ceil(daxis/dbin))
        dbin  = daxis/nbins
        dbins = daxis*(np.arange(nbins)+.5)/nbins
        dists = daxis*s.pos_unit()[:,axis]
        hist  = np.zeros((nbins,),dtype=int)
        boxhist = np.zeros((nbins,),dtype=int)
        ihist = obj()
        iboxhist = obj()
        index = 0
        for d in dists:
            ibin = int(np.floor(d/dbin))
            hist[ibin]+=1
    ...
    # test needed
    #mth
    def layer_composition(self, layers) -> obj:
        lcomp = obj()
    ...
    def shells(
        ...
        *,
        ...
        if identifiers=='point_defects':
            if 'point_defects' not in self:
                msg = 'requested shells around point defects, but structure has no point defects'
                raise ValueError(msg)
            #end if
            core = []
            ...
            bulk = self.pos[bulk_ind]
        #end if
        bulk_ind = np.array(bulk_ind,dtype=int)
        ...
        for n in range(len(dist)):
            ...
            #end if
        #end for
        dshells = np.array(dshells,dtype=float)
        results = [shells]
        if cumshells:
            ...
            for ns,cshell in cumshells.items():
                cumshells[ns] = np.array(cshell,dtype=int)
            #end for
            results.append(cumshells)
        #end if
        for ns,shell in shells.items():
            shells[ns] = np.array(shell,dtype=int)
        if distances:
    ...
    def connected_graphs(
        ...
        **spec_max,  #th
        ...
        if voronoi:
            ...
            nneigh = np.zeros((num_indices,),dtype=int)
            if len(spec_max)>0:
        ...
        #end if
        neigh_table = np.array(neigh_table,dtype=int)
        ...
        if order>0:
            cg = cgraphs[1]
            for i in range(num_indices):  # list of single atoms
                gi = (i,)              # graph indices
        ...
        if actual_indices:
            for o,cg in cgraphs.items():
                cgraphs[o] = np.array(cg,dtype=int)
            #end for
        else:
            ...
            for o,cg in cgraphs.items():
                ...
                for gi in cg:
                    #gi = np.array(gi)
                    gimap = tuple(sorted(indices[np.array(gi)]))
                    cgmap.append(gimap)
                #end for
                cgraphs[o] = np.array(sorted(cgmap),dtype=int)
            #end for
        #end if
    ...
    #mth
    def ring_graphs(self, order, **kwargs) -> obj:
        ...
        for o in range(3,order+1):
            ...
            if lg1 is not None and lg2 is not None:
                ...
                for end_key,llist1 in lg1.items(): # list of lines sharing endpoints
                    ...
                    #end if
                #end for
                rgraphs[o] = np.array(sorted(rg),dtype=int)
            #end if
        #end for
        return rgraphs
    #end def ring_graphs
    ...
    def min_image_centroid(self, points = None, indices = None):
        if indices is not None:
            ...
            msg = 'points or images must be provided to min_image_centroid'
            raise ValueError(msg)
        #end if
    ...
    #mth
    def min_image_centroids(self, points = None, indices = None) -> np.ndarray:
        cents = []
    ...
    def min_image_vectors(
        ...
        *,
        ...
        if pairs:
            vtable = np.empty((npoints,npoints2,self.dim),dtype=float)
            i=-1
            for p in points:
                i+=1
                j=-1
                for pp in points2:
                    j+=1
                    u = dot(pp-p,axinv)
                    vtable[i,j] = dot(u-np.floor(u+.5),axes)
                #end for
            #end for
            result = vtable
        else:
            if npoints!=npoints2:
                msg = (
                    "cannot create one to one minimum image vectors, point sets differ in length\n"
                    f"  npoints1 = {npoints}\n"
                    f"  npoints2 = {npoints2}"
                    )
                raise ValueError(msg)
            ...
            for p in points:
                pp = points2[n]
                u = dot(pp-p,axinv)
                vectors[n] = dot(u-np.floor(u+.5),axes)
                n+=1
            #end for
            result = vectors
        #end if
    ...
    # test needed
    #mth
    def min_image_norms(self, points, norms: int | float) -> list:
        ...
        nout = []
        for p in norms:
            nout.append( ((np.abs(vtable)**p).sum(rdim))**(1./p) )
        #end for
        if len(norms)==1:
            nout = nout[0]
        #end if
    ...
    def voronoi_neighbors(
        ...
        *,
        ...
        d = 3
        t = tuple(np.zeros((d,),dtype=int)+3)
        ss = sn.tile(t)
        ...
        if distance_ordered:
            ...
            for i,ni in neighbors.items():
                ni = np.array(list(set(ni)),dtype=int)
                di = dt[i,ni]
                order = di.argsort()
            ...
            for i,ni in neighbors.items():
                neighbors[i] = np.array(list(set(ni)),dtype=int)
            #end for
        #end if
        return neighbors
    ...
    #mth
    def voronoi_vectors(self, indices = None, restrict = None) -> obj:
    ...
    #mth
    def voronoi_radii(self, indices = None, restrict = None) -> obj:
    ...
    def voronoi_species_radii(self) -> obj:
    ...
    def nearest_neighbors(
        ...
        **spec_max,  #th
        ...
        if spec==elem or rmax is not None or voronoi:
            ...
            msg = (
                "must specify nmax for all species\n"
                f"  species present: {sorted(elem)}\n"
                f"  you only provided nmax for these species: {sorted(spec)}"
                )
            raise ValueError(msg)
        #end if
        pos = self.pos[indices]
    ...
    def chemical_coordination(
        ...
        **spec_max,  #th
        ...
        chem_coord = np.zeros((len(indices),len(chem_key)),dtype=int)
        for i in range(len(indices)):
            counts = np.zeros((len(chem_key),),dtype=int)
        ...
        for coord,ind in chem_map.items():
            chem_map[coord] = np.array(ind,dtype=int)
        #end for
    ...
    # test needed
    #mth
    def rcore_max(self, units = None) -> float | None:
    ...
    # test needed
    #mth
    def cell_image(self, p, center = None) -> np.ndarray:
    ...
    def recenter_k(
        ...
        kaxes                                           = None,  #th
        kcenter           : tuple[int, int, int] | None = None,
        *,
    ...
    # test needed
    #mth
    def recorner(self, center = None) -> None:
        ...
        *,
        ...
        while i<n:
            ...
            while j<n:
                ...
                match = False
                for d in drange:
                    match = match or np.abs(du[d]-1.)<tol
                #end for
                if match:
                    surface[j]=surface[-1]
                    surface.pop()
    ...
    #mth
    def tile(self, *td, **kwargs) -> Structure:
        ...
        ncells = int(np.round( np.abs(det(tilematrix)) ))

        if ncells==1 and np.abs(tilematrix-np.identity(self.dim)).sum()<1e-1:
            if in_place:
                return self
            else:
        ...
        elem = np.array(ncells*list(self.elem))
    ...
    def tile_points(
        ...
        points  : np.ndarray,
        axes    : np.ndarray,
        tilemat : np.ndarray | tuple[int, int, int],
        tilevec : np.ndarray | None = None,
        ) -> np.ndarray:
        if tilevec is None:
            tilemat,tilevec = reduce_tilematrix(tilemat)
        #end if
        if not isinstance(tilemat,np.ndarray):
            tilemat = np.array(tilemat)
        #end if
        matrix_tiling = np.abs(tilemat-np.diag(np.diag(tilemat))).sum()>0.1
        if not matrix_tiling:
            return self.tile_points_simple(points,axes,np.diag(np.abs(tilemat)))
        else:
            if not isinstance(axes,np.ndarray):
                axes = np.array(axes)
            #end if
            if not isinstance(tilevec,np.ndarray):
                tilevec = np.array(tilevec)
            #end if
            dim     = len(axes)
            npoints = len(points)
    ...
    def tile_points_simple(
        ...
        #end if
        t = tilevec
        ti = np.array(np.around(t),dtype=int)
        noninteger = np.abs(t-ti).sum()>1e-6
        if noninteger:
            tp = t.prod()
            if np.abs(tp-int(tp))>1e-6:
                msg = (
                    "tiling vector does not correspond to an integer volume change\n"
                    f"tiling vector: {tilevec}\n"
                    f"volume change: {tilevec.prod()}  {npoints}  {int(npoints)}"
                    )
                raise ValueError(msg)
            #end if
        ...
        if ntpoints==0:
            ...
            tpoints = np.empty((ntpoints,dim))
            ns=0
            ne=npoints
            for k in range(t[2]):
                for j in range(t[1]):
                    for i in range(t[0]):
                        v = dot(np.array([[i,j,k]]),axes)
                        for d in range(dim):
    ...
    #mth
    def opt_tilematrix(self, *args: int, **kwargs) -> tuple[np.ndarray, np.float64]:
        return optimal_tilematrix(self.axes,*args,**kwargs)
    #end def opt_tilematrix

    #mth
    def tile_opt(self, *args: int, **kwargs) -> Crystal:
    ...
    def kfold(
        ...
        kw     = np.array(ncells*list(kweights),dtype=float)/ncells
        return kp,kw
    #end def kfold


    def get_smallest(self) -> Structure | None:
        if self.has_folded():
    ...
    # test needed
    #mth
    def fold(self, small, *requests) -> list:
        msg = 'fold needs a developers attention to make it equivalent with tile'
        raise NotImplementedError(msg)
        if self.dim!=3:
        ...
        corners = []
        ndim = len(small.axes)
        imin = np.empty((ndim,),dtype=int)
        imax = np.empty((ndim,),dtype=int)
        imin[:] =  1000000
        imax[:] = -1000000
        ...
        for k in -1,2:
            for j in -1,2:
                for i in -1,2:
                    ...
                    for d in range(ndim):
                        imin[d] = min(int(np.floor(u[0,d])),imin[d])
                        imax[d] = max(int(np.ceil(u[0,d])),imax[d])
                    #end for
                #end for
            #end for
        #end for
        ...
        small.recenter_k()
        kpoints = np.array(small.kpoints)
        if len(requests)>0:
            results = []
            for request in requests:
                if request=='kmap':
                    ...
                    for i in range(len(kpoints)):
                        ...
                        kmap[kp].append(np.array(kpoints[i]))
                    #end for
        ...
        *,
        ...
        if small is None:
            if self.folded_structure is not None:
                ...
                return np.identity(self.dim,dtype=int)
            #end if
        #end if
        tm = dot(self.axes,inv(small.axes))
        tilemat = np.array(np.around(tm),dtype=int)
        error = np.abs(tilemat-tm).sum()
        ...
        if status:
            ...
            if non_integer_elements:
                msg = (
                    "large cell cannot be constructed as an integer tiling of the small cell\n"
                    "large cell axes:\n"
                    +str(self.axes)+"\n"
                    "small cell axes:  \n"
                    +str(small.axes)+"\n"
    ...
    def primitive(
        ...
        **kwargs,  #th
        ...
        if source not in allowed_sources:
            msg = (
                "source used to obtain primitive cell is unrecognized\n"
                f"source requested: {source}\n"
        ...
        if source=='seekpath':
            ...
            msg = (
                f'primitive source "{source}" is not implemented\n'
    ...
    def add_kpoints(
        ...
        *,
        unique    : bool              = False,
        recenter  : bool              = True,
        cell_unit : bool              = False,
        ) -> None:
        if kweights is None:
            kweights = np.ones((len(kpoints),))
        #end if
        if cell_unit:
            kpoints = np.dot(np.array(kpoints),self.kaxes)
        #end if
        self.kpoints  = np.append(self.kpoints,kpoints,axis=0)
        self.kweights = np.append(self.kweights,kweights)
        if unique:
            self.unique_kpoints()
        #end if
        if recenter:
    ...
    def kgrid_from_kspacing(self, kspacing: float) -> tuple:
        kgrid = []
    ...
    def add_kmesh(
        ...
        *,
        ...
        if kspacing is not None:
            ...
            msg = 'kgrid input is required by add_kmesh'
            raise ValueError(msg)
        #end if
    ...
    def add_symmetrized_kmesh(
        ...
        kshift = np.array(kshift,dtype=float)
        okgrid = 1.0/np.array(kgrid,dtype=float)
        kpoints = np.empty(kpoints_int.shape,dtype=float)
        ...
        #end for
        nkpoints = len(kwmap)
        kpoints_symm  = np.empty((nkpoints,self.dim),dtype=float)
        kweights_symm = np.empty((nkpoints,),dtype=float)
        n = 0
    ...
    def kpoints_unit(self, kpoints: np.ndarray | None = None) -> np.ndarray:
        if kpoints is None:
            kpoints = self.kpoints
        #end if
    ...
    #mth
    def kpoints_reduced(self, kpoints = None) -> float:
        if kpoints is None:
            kpoints = self.kpoints
        #end if
    ...
    #mth
    def kpoints_qmcpack(self, kpoints = None) -> np.ndarray | None:
        if kpoints is None:
    ...
    # test needed
    def inversion_symmetrize_kpoints(
        self,
        tol    : float = 1e-10,
        *,
        folded : bool  = False,
        ...
        pairs = set()
        keep = np.empty((len(kp),),dtype=bool)
        keep[:] = True
        for i in range(len(dtable)):
    ...
    def unique_points_fast(
        ...
        if npoints>0:
            ...
            #end if
            keep = np.ones((npoints,),dtype=bool)
            # place all the points in the box, converted to unit coords
            upoints = np.array(points)
            axinv = inv(axes)
            for i in range(len(points)):
                u = dot(points[i],axinv)
                upoints[i] = u-np.floor(u)
            #end for
    ...
    def kmap(self) -> obj:
        ...
        if self.folded_structure is not None:
            ...
            self.kpoints  = np.array(fs.kpoints)
            self.kweights = np.array(fs.kweights)
    ...
    def select_twist(
        ...
        if isinstance(selector,str):
            ...
            ku_sel = np.array(selector,dtype=float)
            n = 0
            ...
            if index is None:
                msg = (
                    "cannot identify twist number\n"
                    f"twist requested: {ku_sel}\n"
                    f"twists present: {sorted([tuple(k) for k in self.kpoints_unit()])}"
                    )
                raise KeyError(msg)
            #end if
        else:
        ...
        if invalid_selector:
            msg = (
                "cannot identify twist number\n"
                f"invalid selector provided: {selector}\n"
                "valid string inputs for selector: smallest, random\n"
                "selector can also be a length 3 tuple, list or array (a twist vector)"
                )
            raise ValueError(msg)
        #end if
        return index
    ...
    # test needed
    #mth
    def fold_pos(self, large, tol: float = 0.001) -> None:
        vratio = large.volume()/self.volume()
        if np.abs(vratio-int(np.around(vratio)))>1e-6:
            msg = (
                "cannot fold positions from large cell into current one\n"
                "large cell volume is not an integer multiple of the current one\n"
                f"large cell volume: {large.volume()}\n"
                f"current cell volume: {self.volume()}\n"
                f"volume ratio: {vratio}"
                )
            raise ValueError(msg)
        ...
        if dt.ravel().max()>tol:
            msg = (
                ...
                f"max distance encountered: {dt.ravel().max()}\n"
                f"tolerance: {tol}"
                )
            raise RuntimeError(msg)
        #end if
        ...
        if (counts!=nnearest).any():
            msg = (
                "cannot fold positions from large cell into current one\n"
                f"each atom must have {nnearest} equivalent positions\n"
                f"some atoms found with the following equivalent position counts: {counts[counts!=nnearest]}"
                )
            raise RuntimeError(msg)
        #end if
        ind_visited = set()
    ...
    #mth
    def pos_unit(self, pos = None) -> np.ndarray:
        if pos is None:
            pos = self.pos
        #end if
    ...
    def pos_to_cartesian(self) -> None:
        self.pos = dot(self.pos,self.axes)
    #end def pos_to_cartesian


    # test needed
    def at_Gpoint(self) -> bool | np.bool_:
    ...
    # test needed
    def at_real_kpoint(self) -> bool:
        kpu = 2*self.kpoints_unit()
        return len(kpu)==1 and np.abs(kpu-np.around(kpu)).sum()<1e-6
    ...
    def bonds(
        ...
        *,
        ...
        if self.dim!=3:
            msg = 'bonds is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        natoms,dim = self.pos.shape
        centers = np.empty((natoms,neighbors,dim))
        distances = np.empty((natoms,neighbors))
        vect      = np.empty((natoms,neighbors,dim))
        t = self.tile((3,3,3))
    ...
    def displacement(
        ...
        *,
        ...
        if self.dim!=3:
            msg = 'displacement is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        ref = reference.tile((3,3,3))
        ref.recenter(reference.center)
        rmap = np.array(3**3*list(range(len(reference.pos)),dtype=int))
    ...
    # test needed
    #mth
    def distortion(self, reference, neighbors) -> tuple[np.ndarray, np.ndarray]:
        if self.dim!=3:
            msg = 'distortion is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        ...
        distortion = np.empty(sbv.shape)
        magnitude  = np.empty((len(self.pos),))
        for i in range(len(self.pos)):
            ir = nn[i]
            bonds  = sbv[i]
            rbonds = rbv[ir]
            ib  = np.empty((neighbors,),dtype=int)
            ibr = np.empty((neighbors,),dtype=int)
    ...
    def bond_compression(self, reference, neighbors):
        ...
        bondlen = rbl.mean()
        return np.abs(1.-sbl/bondlen).max(axis=1)
    ...
    def embed(
        ...
        replaced = np.empty((len(self.pos),),dtype=bool)
        replaced[:] = False
        ...
        for i in range(len(pos)):
            ...
            #end if
        #end for
        remove = np.arange(len(self.pos))[inside & np.logical_not(replaced)]
        remove.sort()
        remove = np.flipud(remove)
        for i in remove:
            elist.pop(i)
            plist.pop(i)
        #end for
    ...
    def shell(
        ...
        if self.dim!=3:
            msg = 'shell is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        ...
        ncells=3**3
        ntile = ncells*natoms
        pos = np.empty((ntile,dim))
        ind = np.empty((ntile,),dtype=int)
    ...
    def interpolate(
        ...
        *,
        ...
        if (s1.elem!=s2.elem).any():
            msg = (
                "cannot interpolate structures, atoms do not match\n"
                f"  atoms1: {s1.elem}\n"
                f"  atoms2: {s2.elem}"
                )
            raise ValueError(msg)
        ...
        pos1 = s1.pos
        pos2 = s2.pos
        min_image &= np.abs(ax1-ax2).max()<1e-6
        if min_image:
    ...
    #mth
    def madelung(self, axes = None, tol: float = 1e-10) -> np.float64:
        ...
        for n in range(1,nshells+1):
            ...
            vshell.append(0.)
            for d in domains:
                vshell[n] += (erfc(kconv*d.R)/d.R).sum() + 4*pi/volume*(np.exp(gconst*d.G2)/d.G2).sum()
            #end for
            if np.abs(vshell[n]-vshell[n-1])<tol:
                break
            #end if
        #end for
        vm = vmc + vshell[-1]
    ...
    def makov_payne(
        ...
        if order!=1:
            msg = 'Only first order Makov-Payne correction is currently supported.'
            raise NotImplementedError(msg)
        #end if
    ...
    def read(
        ...
        *,
        ...
        #end if
        if format is None:
            msg = 'file format must be provided'
            raise ValueError(msg)
        #end if
        self.mag    = None
        self.frozen = None
    ...
    def read_xyz(self, filepath: str) -> None:
        ...
        if len(lines)>1:
            ...
            if len(lines)>2:
                for l in lines[2:]:
                    ...
                    if len(tokens)==4:
                        ...
                        pos.append(np.array(tokens[1:],float))
                        natoms+=1
                        if natoms==ntot:
                            break
                        #end if
            ...
            if natoms!=ntot:
                msg = (
                    "xyz file read failed\n"
                    f"attempted to read file: {filepath}\n"
                    f"number of atoms expected: {ntot}\n"
                    f"number of atoms found: {natoms}"
                    )
                raise FileFormatError(msg)
            #end if
        #end if
    ...
    def read_xsf(self, filepath: str | XsfFile) -> None:
    ...
    def read_poscar(self, filepath: str, elem: list[str] | None = None) -> None:
        if os.path.exists(filepath):
            with open(filepath, "r") as f:
        ...
        if nlines<min_lines:
            msg = (
                f"POSCAR file must have at least {min_lines} lines\n"
        ...
        if tokens[0].isdigit():
            ...
            if elem is None:
                ...
                msg = (
                    ...
                    f"  number of elem counts: {len(counts)}\n"
                    f"  number of elem given: {len(elem)}"
                    )
                raise ValueError(msg)
            #end if
            lcur = 6
        ...
        if selective_dynamics or spos.shape[1]>3:
            move = np.array(spos[:,3:6],dtype=str)
    ...
    def read_fhi_aims(self, filepath) -> None:
        ...
        for line in lines:
            ...
            if len(ls)>0 and ls[0]!='#':
                ...
                if t0=='lattice_vector':
                    ...
                    #None
                    msg = (
                        f"unrecogonized or not yet supported token in fhi-aims geometry file: {t0}"
                        )
                    raise NotImplementedError(msg)
                #end if
            #end if
        ...
        if len(constrain_relax)>0:
            constrain_relax = np.array(constrain_relax)
            self.freeze(list(range(self.size())),directions=constrain_relax=='.true.')
        #end if
    #end def read_fhi_aims

    #mth
    def write(self, filepath: str | Path | None = None, format = None) -> str:
        if filepath is None and format is None:
            msg = 'please specify either the filepath or format arguments to write()'
            raise ValueError(msg)
        elif format is None:
        ...
        if format=='xyz':
            ...
            msg = f'file format {format} is unrecognized'
            raise NotImplementedError(msg)
        #end if
        return c
    #end def write
    ...
    def write_xyz(self, filepath: str | Path | None = None) -> str:
        ...
        """
        if self.dim!=3:
            msg = 'write_xyz is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        s = deepcopy(self)
        s.change_units("A")
        ...
        for i in range(len(s.elem)):
            e = s.elem[i]
            p = s.pos[i]
            c+=f' {e:2} {p[0]:12.8f} {p[1]:12.8f} {p[2]:12.8f}\n'
        #end for
    ...
    def write_xsf(self, filepath: str | Path | None = None) -> str:
        if self.dim!=3:
            msg = 'write_xsf is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
        s = deepcopy(self)
        s.change_units('A')
        ...
        for i in range(len(s.elem)):
            ...
            if not identified:
                msg = (
                    f"{e} is not an element\n"
                    "xsf file cannot be written"
                    )
                raise NotAnElementError(msg)
            #end if
            enum = element.atomic_number
            r = s.pos[i]
            c += f'   {enum:>3} {r[0]:12.8f}  {r[1]:12.8f}  {r[2]:12.8f}\n'
        #end for
        if filepath is not None:
            with open(filepath, "w") as f:
                f.write(c)
    ...
    def write_poscar(self, filepath: Path | None = None) -> str:
        s = deepcopy(self)
        s.change_units('A')
    ...
    # test needed
    #mth
    def write_fhi_aims(self, filepath = None) -> str:
        s = deepcopy(self)
        s.change_units('A')
        c = ''
        c+='\n'
        ...
        for p,e in zip(self.pos,self.elem):
            c += f'atom_frac   {p[0]: 12.8f}  {p[1]: 12.8f}  {p[2]: 12.8f}  {e}\n'
        #end for
        if filepath is not None:
            with open(filepath, "w") as f:
                f.write(c)
    ...
    def plot2d_ax(
        ...
        **kwargs,  #th
        ...
        pp = np.array([0*a,ax[ix],ax[ix]+ax[iy],ax[iy],0*a])
    ...
    def plot2d(self, pos_style: str = 'b.', ax_style: str = 'k-') -> None:
        import matplotlib.pyplot as plt
        if self.dim!=3:
    ...
    def show(self, viewer: str = 'vmd', filepath: str = '/tmp/tmp.xyz') -> None:
        if self.dim!=3:
            msg = 'show is currently only implemented for 3 dimensions'
            raise NotImplementedError(msg)
        #end if
    ...
    def get_atomic_numbers(self) -> np.ndarray:
        ...
        for e in self.elem:
            ...
            if not iselem:
                msg = f'Atomic symbol, {element}, not recognized'
                raise NotAnElementError(msg)
            else:
    ...
    def get_magnetic_moments(self) -> None:
        msg = 'structure objects do not currently support magnetic moments'
        raise NotImplementedError(msg)
    ...
    #mth
    def get_symmetry(self, symprec: float = 1e-5):
        import spglib
        cell = self.spglib_cell()
    ...
    #mth
    def symmetry_data(self, *args, **kwargs) -> obj:
        import spglib
        ...
        if isinstance(ds,dict):
            ...
            raise TypeError(f'Invalid symmetry dataset type: {type(ds)}')
        for k,v in ds.items():
    ...
    def bravais_lattice_name(self, symm_data: obj | None = None) -> str:
        ...
        if not isinstance(sg,int) or sg<1 or sg>230:
            msg = f'Invalid space group from spglib: {sg}'
            raise ValueError(msg)
        #end if
        if not isinstance(name,str):
            msg = f'Invalid space group name from spglib: {name}'
            raise RuntimeError(msg)
        #end if
        bv = None
        if sg>=1 and sg<=2:
        ...
        if bv is None:
            msg = (
                ...
                f"Space group number and name: {sg} {name}"
                )
            raise RuntimeError(msg)
        #end if
        return bv
    ...
    #mth
    def check_point_group_operations(
        self,
        rotations         = None,  #th
        tol       : float = 1e-5,
        dtol      : float = 1e-5,
        ...
        for sn,sset in species_by_specnum.items():
            if len(sset)>1:
                msg = (
                    ...
                    f"Species marked in this way: {list(sset)}"
                    )
                raise RuntimeError(msg)
            #end if
        ...
        *,
        ...
        if rmg_lattice is None and allow_general:
            ...
            if s_trans.units=='A':
                ...
                msg = f'Unrecognized length units in structure "{s_trans.units}"'
                raise ValueError(msg)
            #end if
            ...
            if bl_type=='Cubic Primitive':
                ...
                a = norm(axes[0])*2/np.sqrt(3.)
                b = a
                c = a
            elif bl_type=='Cubic Face Centered':
                a = norm(axes[0])*2/np.sqrt(2.)
                b = a
                c = a
                ...
                msg = f'Unrecognized RMG bravais_lattice_type "{bl_type}"'
                raise ValueError(msg)
            #end if
            rmg_inputs = obj(
...
def _getseekpath(
    ...
    angle_tolerance    : float            = 1.0,
    *,
    with_time_reversal : bool             = False,
    ):  #th
    from seekpath import get_explicit_k_path
    if not isinstance(structure, Structure):
        raise TypeError(
            "structure is not of type Structure\n"
            f"type received: {structure.__class__.__name__}"
            )
    #end if
...
def get_band_tiling(
    *,
    ...
    check_standard : bool                     = True,
    use_ktol       : bool                     = True,
    kpoints_label                             = None,  #th
    kpoints_rel    : list[list[float]] | None = None,
    max_volfac     : int                      = 20,
    min_volfac     : int                      = 0,
    target_volfac                             = None,  #th
    ) -> obj | None:
    #mth
    def cube_deviation(axes):
        a = axes
        volume = np.abs(dot(cross(axes[0,:], axes[1,:]), axes[2,:]))
        dc = volume**(1./3)*sqrt(2.)
        d1 = np.abs(norm(a[0]+a[1])-dc)
        d2 = np.abs(norm(a[1]+a[2])-dc)
        d3 = np.abs(norm(a[2]+a[0])-dc)
    ...
    def cuboid_with_int_edges(vol: int) -> list[int | list[int] | np.ndarray]:
        ...
        if isinstance(vol, int):
            ...
            msg = 'Volume multiplier must be integer'
            raise ValueError(msg)
        #end if
    ...
    def alphas_on_grid(
        alphas : np.ndarray,
        divs   : list[int | list[int]],
        ) -> list[np.ndarray]:
        new_alphas = []
        for alpha in alphas:
            abs_alpha  = np.abs(alpha)
            sign_alpha = np.sign(alpha)
            new_alpha  = np.round(abs_alpha*divs)*1./divs*sign_alpha
    ...
    def find_alphas(
        ...
        if kpoints_label is None:
            ...
            if kpoints_rel is None:
                ...
                for k in kpoints_rel:
                    ...
                    if any(kindex):
                        ...
                        msg = f'{k} is not found in the kpath'
                        raise RuntimeError(msg)
                    #end if
                #end for
            #end if
            ...
            if kpoints_rel is not None:
                msg = 'Both symbolic and crystal k-points are defined.'
                raise ValueError(msg)
            else:
                kpoints_rel = []
                num_kpoints = 0
            ...
            #end if
        #end if
        alphas = np.array([x[0] - x[1] for x in itertools.combinations(kpts.values(),2)]) #Combinations of k_1 - k_2 in kpts list
    ...
    def find_vars(
        ...
        if target_volfac is not None:
            if min_volfac is None and max_volfac is None:
                ...
                msg = "target_volfac and {min_volfac, max_volfac} cannot be defined together!"
                raise ValueError(msg)
            #end if
        #end if
        ...
        for mvol in range(max_volfac, 0, -1):
            ...
            for c in cuboids:
                ...
                rec_grid_voxel = np.array([1./a_1,1./a_2,1./a_3]) # reciprocal grid voxel size
                rem = []
                for alpha in alphas:
                    rem.append(np.mod(np.abs(alpha), rec_grid_voxel))
                #end for
                if np.all(np.isclose(rem,0.0,atol=ktol)):
                    n1     = a_1
                    n2     = a_2
                    n3     = a_3
        ...
        if vars == []:
            ...
            while not can_be_found:
                if volfac*vol_mul <= max_volfac and volfac*vol_mul >= min_volfac:
                    ...
                    msg = 'Increase max_volfac or target_volfac!'
                    raise ValueError(msg)
                else:
                    vol_mul+=1
                #end if
                ...
                for q in range(0, g12//g123):
                    for r in range(g13*g23//g123):
                        temp_mat = [
                            ...
                            [0,0,n3]]
                        comm = []
                        div = np.array([n1, n2, n3])
                        new_alphas = alphas_on_grid(alphas, div)
                        for new_alpha in new_alphas:
                            if (np.isclose(np.abs(dot(temp_mat,new_alpha))%1.0, 0, atol = 1e-6)).all():
    ...
    #mth
    def find_shift(final_mat, structure, kpt0) -> None:
        return None
    #end def find_cubic_mat
...
def interpolate_structures(
    struct1   : Structure | list | tuple,
    struct2   : Structure | None = None,
    *,
    images                       = None,  #th
    min_image : bool             = True,
    recenter  : bool             = True,
    ...
    if repackage:
        if system1 is not None:
            ...
            msg = 'cannot repackage into physical systems since no system object was provided in place of a structure'
            raise ValueError(msg)
        #end if
        systems = []
        for s in structures:
...
# test needed
#mth
def structure_animation(filepath, structures, tiling = None) -> None:
    path,file = os.path.split(filepath)
    if not file.endswith('xyz'):
        msg = 'only xyz files are supported for now'
        raise NotImplementedError(msg)
    #end if
    anim = ''
    for s in structures:
...
class DefectStructure(Structure):
    ...
    #mth
    def defect_from_bond_compression(
        self,
        compression_cutoff,  #th
        bond_eq,  #th
        neighbors,  #th
        ) -> Structure:
        bind,bcent,blens = self.bonds(neighbors)
        ind = bind[ np.abs(blens/bond_eq - 1.) > compression_cutoff ]
        idefect = np.array(list(set(ind.ravel())))
        defect = self.carve(idefect)
        return defect
    #end def defect_from_bond_compression

    #mth
    def defect_from_displacement(self, displacement_cutoff, reference) -> Structure:
    ...
    def compare(
        ...
        ismall = np.array(list(range(len(dsmall.pos))))
    ...
    def __init__(
        ...
        *,
        ...
        if constants is None:
            msg = 'the variable constants must be provided'
            raise ValueError(msg)
        #end if
        if atoms is None:
            msg = 'the variable atoms must be provided'
            raise ValueError(msg)
        #end if
        ...
        if len(cscale)!=len(constants):
            msg = 'cscale and constants must be the same length'
            raise ValueError(msg)
        #end if
        basis  = np.array(basis)
        tiling = np.array(tiling,dtype=int)
        cscale = np.array(cscale)
        constants = cscale*np.array(constants)
        ...
        a2c = b*np.array([cg,sg,0])
        a3c = c*np.array([cb,y,sqrt(sb**2-y**2)])
        #a1c = np.array([a,0,0])
        #a2c = np.array([b*cos(gamma),b*sin(gamma),0])
        #a3c = np.array([c*cos(beta),c*cos(alpha)*sin(beta),c*sin(alpha)*sin(beta)])
        axes_conv = np.array([a1c,a2c,a3c]).copy()
        ...
        if axes is None:
            ...
            axes = dot(np.diag([a,b,c]),np.array(axes))
        #end if
...
class Jellium(Structure):
    ...
    def __init__(
        ...
        if rs is not None:
            if dim not in self.prefactors:
                msg = (
                    ...
                    f"  you requested one with dimension {dim}"
                    )
                raise ValueError(msg)
            #end if
        ...
        if cell is not None:
            ...
            volume = float(volume)
            cell   = volume**(1./dim)*np.identity(dim)
        #end if
        ...
        if charge is None or cell is None:
            msg = (
                "not enough information to form jellium structure\n"
...
def generate_cell(
    ...
    scale       : float = 1.,
    units               = None,  #th
    struct_type         = Structure,  #th
    ):  #th
    if tiling is None:
        tiling = (1,1,1)
    #end if
    axes = Sobj()
    axes.sc  =  1.*np.array([[ 1,0,0],[0, 1,0],[0,0, 1]])
    axes.bcc = .5*np.array([[-1,1,1],[1,-1,1],[1,1,-1]])
    axes.fcc = .5*np.array([[ 1,1,0],[1, 0,1],[0,1, 1]])
    ax     = dot(np.diag(tiling),axes[shape])
    center = ax.sum(0)/2
...
#mth
def generate_structure(type: str = 'crystal', *args, **kwargs) -> Structure:
    if type=='crystal':
...
def generate_dimer_structure(
    *,
    ...
    """
    if dimer is None:
        msg = 'dimer atoms must be provided to construct dimer'
        raise ValueError(msg)
    #end if
    if separation is None:
        msg = 'separation must be provided to construct dimer'
        raise ValueError(msg)
    #end if
    if Lbox is not None:
        axes = [[Lbox*(1-skew),0,0],[0,Lbox,0],[0,0,Lbox*(1+skew)]]
...
def generate_trimer_structure(
    *,
    trimer        : list[str] | None                 = None,
    units         : str                              = 'A',
    separation    : list[float] | None               = None,
    ...
    """
    if trimer is None:
        msg = 'trimer atoms must be provided to construct trimer'
        raise ValueError(msg)
    #end if
    if separation is None:
        msg = 'separation must be provided to construct trimer'
        raise ValueError(msg)
    #end if
    if len(separation)!=2:
        msg = (
            "two separation distances (atom1-atom2,atom1-atom3) must be provided to construct trimer\n"
            f"you provided {len(separation)} separation distances"
            )
        raise ValueError(msg)
    #end if
    if angle is None:
        msg = 'angle must be provided to construct trimer'
        raise ValueError(msg)
    #end if
    if angular_units=='degrees':
        angle *= pi/180
...
# test needed
#mth
def generate_jellium_structure(*args, **kwargs) -> Jellium:
...
def generate_defect_structure(
    *,
    ...
    kshift      : tuple[int, int, int] = (0,0,0),
    units                              = None,  #th
    struct_type                        = DefectStructure,  #th
    ):  #th
    if structure in defects:
        dstruct = defects[structure]
    else:
        msg = 'defects for '+structure+' structure have not yet been implemented'
        raise NotImplementedError(msg)
    #end if
    if defect in dstruct:
        drep = dstruct[defect]
    else:
        msg = defect+' defect not found for '+structure+' structure'
        raise RuntimeError(msg)
    #end if
